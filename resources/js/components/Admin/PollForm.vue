<template>
    <div class="bg-purple-900 px-4 p-4">
        <input v-model="form.question" type="text" class="form-input w-full border-0 shadow bg-purple-700 mb-4 text-white" placeholder="Question">
        <div v-for="(option, index) in form.options">
            <input v-model="form.options[index]" type="text" class="form-input w-full border-0 shadow bg-purple-700 my-1 text-white" :placeholder="'Option ' + (index + 1)">
        </div>
